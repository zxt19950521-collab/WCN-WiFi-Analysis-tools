...
class WifiLogAnalyzerWithTags:
    ...
    def load_tags(self):
        """加载TAG配置"""
        tag_config_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tag_config.json")
        ...
        for main_log in self.main_logs:
            ...
            with open(main_log, 'r', encoding='utf-8', errors='ignore') as f:
                for line in f:
                    ...
                    # 为事件添加TAG
                    event_tags = self.get_event_tags(line)
    ...
    def get_event_tags(self, line):
        """根据日志内容为事件添加TAG"""
        event_tags = []
        
        # 根据日志内容匹配TAG
        if 'setWifiEnabled' in line or 'Starting AIDL supplicant' in line:
            event_tags.append('开关')
        elif 'startScan' in line or 'Scan completed' in line:
            event_tags.append('扫描')
        elif 'connect uid' in line or 'Trying to associate' in line or 'Associated with' in line:
            event_tags.append('连接')
        elif 'DhcpClient' in line:
            event_tags.append('DHCP')
        elif 'SoftAp' in line or 'hostapd' in line:
            event_tags.append('热点')
        elif 'p2p' in line.lower() or 'P2P' in line:
            event_tags.append('P2P')
        elif 'AirTransfer' in line or 'NearP2pManager' in line:
            event_tags.append('极速互传')
        elif 'Power Save' in line or 'power' in line.lower():
            event_tags.append('功耗')
        elif 'mtk_cfg80211_get_station' in line or 'rssi' in line.lower() or 'frequency' in line.lower():
            event_tags.append('上网')
        elif 'WifiNetworkSelector' in line or 'WifiConnectivityManager' in line or 'tranwifismartassistant' in line:
            event_tags.append('自动回连&WIFI切换')
    ...
    def plot_connection_process(self):
        ...
        for event in data:
            ...
            # 添加TAG信息
            if event['tags']:
                hover_text += '<b>TAG</b>: '+', '.join(event['tags'])+'<br>'
    ...
    def generate_markdown_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write('| TAG  | 事件数量 |\n')
            f.write('| --- | ---- |\n')
            for tag, count in sorted(tag_statistics.items(), key=lambda x: x[1], reverse=True):
                f.write(f'| {tag} | {count} |\n')
            f.write('\n***\n\n')
            ...
            for i, event in enumerate(issue_events):
                ...
                f.write(f'| {i+1} | {time_str} | {event["message"]} | {tags_str} |\n')
            if not issue_events:
            ...
            if is_airtransfer_issue:
                ...
                for i, event in enumerate(airtransfer_events):
                    tags_str = ', '.join(event.get('tags', []))
                    f.write(f'**{event["timestamp"].strftime("%H:%M:%S")} - {event["type"]}: {event["message"]}** [TAG: {tags_str}]\n\n')
                ...
                if disconnect_events:
                    for i, event in enumerate(disconnect_events):
                        tags_str = ', '.join(event.get('tags', []))
                        f.write(f'### 断连事件：{event["timestamp"].strftime("%H:%M:%S")} {event["message"]} [TAG: {tags_str}]\n\n')
                        f.write('**根因：WiFi连接中断，可能导致文件传输失败。**\n\n')
                        f.write('| 时间           | 事件                                                                          | TAG |\n')
                        f.write('| ------------ | --------------------------------------------------------------------------- | --- |\n')
                        f.write(f'| {event["timestamp"].strftime("%H:%M:%S.%f")[:-3]} | {event["message"]} | {tags_str} |\n')
            ...
            f.write('## 【问题原因总结】\n\n')
            f.write('| 序号  | 时间    | 根因                                                                | 类型     | TAG |\n')
            f.write('| --- | ----- | ----------------------------------------------------------------- | ------ | --- |\n')
            if is_airtransfer_issue:
                for i, event in enumerate(airtransfer_events):
                    time_str = event['timestamp'].strftime('%H:%M:%S')
                    tags_str = ', '.join(event.get('tags', []))
                    f.write(f'| {i+1} | {time_str} | {event["message"]} | 极速互传问题 | {tags_str} |\n')
            else:
                disconnect_events = [e for e in self.events if e['type'] == 'DISCONNECT']
                for i, event in enumerate(disconnect_events):
                    time_str = event['timestamp'].strftime('%H:%M:%S')
                    tags_str = ', '.join(event.get('tags', []))
                    f.write(f'| {i+1} | {time_str} | {event["message"]} | 连接断开   | {tags_str} |\n')
            if not issue_events:
            ...
            if self.events:
                for event in sorted(self.events, key=lambda x: x['timestamp']):
                    time_str = event['timestamp'].strftime('%H:%M:%S')
                    tags_str = ', '.join(event.get('tags', []))
                    f.write(f'  {time_str} - {event["type"]}: {event["message"]} [TAG: {tags_str}]\n')
            else:
                f.write('  未检测到相关事件\n')
            f.write('```\n\n')
...
'''
            for i, event in enumerate(airtransfer_events):
                tags_str = ', '.join(event.get('tags', []))
                html_content += f'''
            <p><strong>{event["timestamp"].strftime("%H:%M:%S")} - {event["type"]}: {event["message"]}</strong> [TAG: {tags_str}]</p>
'''
            html_content += '''
...
'''
            ...
            if disconnect_events:
                for i, event in enumerate(disconnect_events):
                    tags_str = ', '.join(event.get('tags', []))
                    html_content += f'''
            <h3>断连事件：{event["timestamp"].strftime("%H:%M:%S")} {event["message"]} [TAG: {tags_str}]</h3>
...
'''
        ...
        if self.events:
            for event in sorted(self.events, key=lambda x: x['timestamp']):
                time_str = event['timestamp'].strftime('%H:%M:%S')
                tags_str = ', '.join(event.get('tags', []))
                html_content += f'  {time_str} - {event["type"]}: {event["message"]} [TAG: {tags_str}]\n'
        else:
...
'''
        ...
        <div class="section">
            <h2>【信号强度与速率分析】</h2>
            <img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=WiFi%20RSSI%20signal%20strength%20and%20rate%20charts&image_size=landscape_16_9" alt="RSSI and Rate Analysis" style="width:100%;">
        </div>
        <div class="section">
            <h2>【连接时间轴】</h2>
            <img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=WiFi%20connection%20timeline%20chart%20showing%20connection%20attempts%20and%20status&image_size=landscape_16_9" alt="Connection Timeline" style="width:100%;">
        </div>
        <div class="section">
            <h2>【连接过程分析】</h2>
            <img src="https://trae-api-cn.mchost.guru/api/ide/v1/text_to_image?prompt=WiFi%20connection%20process%20timeline%20showing%20auth%20assoc%20handshake%20dhcp%20steps&image_size=landscape_16_9" alt="Connection Process" style="width:100%;">
        </div>
'''
...
if __name__ == "__main__":
    # 测试目录
    test_dir = "Test_log/rebooterro/正常开机/debuglogger/mobilelog/APLog_2026_0412_203846__4"
    analyzer = WifiLogAnalyzerWithTags(test_dir)
    analyzer.analyze()
