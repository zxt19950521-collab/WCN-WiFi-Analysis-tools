...
2. **数据提取**：提取连接、认证、关联、握手、DHCP等关键事件，显示1/4、2/4、3/4、4/4握手状态
3. **详细信息提取**：从事件中提取SSID、MAC地址、节点类型、reason code等详细信息
4. **异常连接检测**：自动检测异常连接过程，用红色标记异常事件
5. **可视化分析**：使用Plotly库生成可交互的HTML图表，包括连接时间轴、连接过程、信号强度和速率变化图表
6. **报告生成**：自动生成包含详细分析的Markdown和HTML格式报告，将图表嵌入到HTML报告中
7. **故障定位**：基于红宝书中的经验，定位具体故障点
